  ...
  if (submitted) {
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="mt-2 text-ink-secondary text-sm">{t('wrapDay')}</p>
        <button
          onClick={() => navigate('/')}
  ...
  return (
    <div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); mutation.mutate() }} className="space-y-6">
        ...
        >
          {mutation.isPending ? t('common:saving') : t('wrapUpTheDay')}
        </button>
      </form>
    </div>
  )
...
export function EndDayPage() {
  ...
  return (
    <div className="min-h-screen bg-surface flex items-start justify-center pt-16 px-4">
      <div className="w-full max-w-lg">
        <h1 className="text-2xl font-semibold text-ink-heading mb-2">{t('endOfDay')}</h1>

        {noItems ? (
          <div className="mb-8">
            <p className="text-ink-secondary text-sm mb-6">{t('inboxIsClear')}</p>
          </div>
        ) : (
          <div className="mb-8">
            <p className="text-sm text-ink-muted mb-4">
              {t('inboxProgress', { current: processedCount + 1, total: totalItemsRef.current })}
            </p>
