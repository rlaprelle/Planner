...
export function TaskDetailModal({ open, onOpenChange, task, projectName, projectId }) {
  const { t } = useTranslation('tasks')
  const queryClient = useQueryClient()
  ...
  function handleStatusChange(val) { setStatus(val); saveField({ status: val }) }
  function handlePriorityChange(val) { const num = val ? Number(val) : null; setPriority(num); saveField({ priority: num }) }
  function handlePointsChange(val) { const num = val ? Number(val) : null; setPointsEstimate(num ?? ''); saveField({ pointsEstimate: num }) }
  function handleEnergyChange(val) { setEnergyLevel(val ?? ''); saveField({ energyLevel: val || null }) }
  function handleDueDateBlur() {
    if ((dueDate || null) !== (task.dueDate || null)) saveField({ dueDate: dueDate || null })
  }
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-40 w-full max-w-lg max-h-[85vh] bg-surface-raised rounded-2xl shadow-modal focus:outline-none flex flex-col overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">
            ...
            <div className="flex items-start gap-2 px-6 py-5 border-b border-edge-subtle flex-shrink-0">
              <div className="flex-1 min-w-0">
                ...
                />
                <p className="text-xs text-ink-muted mt-1">{projectName}</p>
              </div>
              <Dialog.Close asChild>
                <button
                  type="button"
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              <SelectField label={t('status')} value={status} onChange={handleStatusChange} options={STATUS_OPTIONS} />
              <SelectField label={t('priority')} value={priority} onChange={handlePriorityChange} options={PRIORITY_OPTIONS} />
              <SelectField label={t('pointsEstimate')} value={pointsEstimate} onChange={handlePointsChange} options={POINTS_OPTIONS} emptyLabel={t('common:noneSelected')} />
              <SelectField label={t('energyLevel')} value={energyLevel} onChange={handleEnergyChange} options={ENERGY_OPTIONS} emptyLabel={t('common:noneSelected')} />
              ...
              <div>
                ...
                {children.length === 0 ? (
                  ...
                  <div className="space-y-0.5">
                    {children.map((child) => (
                      <ChildTaskItem key={child.id} child={child} projectId={projectId} />
                    ))}
                  </div>
                )}
              </div>
            ...
            {/* Archive button */}
            <div className="px-6 py-4 border-t border-edge-subtle flex-shrink-0">
              <button
                type="button"
